...
class DbMysql extends Lib\Db implements Interfaces\DataBaseInterface
{
    ...
    public function __construct($config=''){
        if ( !extension_loaded('mysql') ) {
            Vpf\halt(Vpf\L('_NOT_SUPPERT_').':mysql');
        }
        if(!empty($config)) {
            $this->config   =   $config;
    ...
    public function connect($config='',$linkNum=0,$force=false) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            if ( !$this->linkID[$linkNum] || (!empty($config['database']) && !mysql_select_db($config['database'], $this->linkID[$linkNum])) ) {
                Vpf\halt(mysql_error());
            }
            $dbVersion = mysql_get_server_info($this->linkID[$linkNum]);
            //使用UTF8存取数据库
            mysql_query("SET NAMES '".Vpf\C('DB_CHARSET')."'", $this->linkID[$linkNum]);
            //设置 sql_model
            if($dbVersion >'5.0.1'){
            ...
            $this->connected    =   true;
            // 注销数据库连接配置信息
            if(1 != Vpf\C('DB_DEPLOY_TYPE')) unset($this->config);
        }
